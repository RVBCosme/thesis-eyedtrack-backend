...
class VoiceAlertManager(private val context: Context) {
    ...
    // Removed BEHAVIOR_RESET_TIME - using consecutive detection instead
    ...
    private fun setupAudio() {
        try {
            ...
            Log.d(TAG, "Audio setup complete. Volume: ${audioManager.getStreamVolume(AudioManager.STREAM_MUSIC)}/${maxVolume}")
        } catch (e: Exception) {
    ...
    fun processAlerts(isDrowsy: Boolean, isYawning: Boolean, isDistracted: Boolean) {
        ...
        processBehavior("drowsy", isDrowsy, currentTime, params) {
            speak("Warning! Drowsiness detected! Please stay alert and consider taking a break.", TextToSpeech.QUEUE_FLUSH, params)
        }

        processBehavior("yawning", isYawning, currentTime, params) {
            speak("Warning! Yawning detected. You may be getting tired. Consider taking a rest.", TextToSpeech.QUEUE_ADD, params)
        }

        processBehavior("distracted", isDistracted, currentTime, params) {
            speak("Warning! Distraction detected. Please focus on the road ahead.", TextToSpeech.QUEUE_ADD, params)
        }
    ...
    fun resetCounters() {
        ...
        behaviorCounters["drowsy"] = 0
        behaviorCounters["yawning"] = 0
        behaviorCounters["distracted"] = 0
        
        behaviorLastSeen.clear()
        behaviorLastSeen["drowsy"] = 0L
        behaviorLastSeen["yawning"] = 0L
        behaviorLastSeen["distracted"] = 0L
        
        Log.d(TAG, "Reset all behavior counters and tracking")
    }
    ...
    fun shutdown() {
        try {
            ...
            Log.d(TAG, "VoiceAlertManager shutdown complete")
        } catch (e: Exception) {
